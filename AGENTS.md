...
- Do NOT import either compiler provider or its `internal/...` packages directly from Effect code.
- Always import the canonical shim packages (`github.com/microsoft/TypeScript/tsc/shim/...`) instead.
...
- All compiler changes should be as minimal as possible and made through the provider-specific patch stack: `_patches/typescript-go/` for the legacy repository or `_patches/typescript/` for the `tsc/` subtree in the TypeScript repository. Compiler checkouts are reset during check runs, so direct edits there are ephemeral unless captured in the matching patch folder.

## Reference Repositories

This repository uses local reference clones under `.repos/` for pattern and implementation research. These repositories are local-only working material and remain gitignored (`.repos/` is ignored by this repo). They are created on demand rather than by `pnpm setup-repo`.

Canonical reference clones:
...
`repoctl perf compare` requires an explicit target repository. The `port-commit` command creates or updates `.repos/effect-language-service` when needed.
