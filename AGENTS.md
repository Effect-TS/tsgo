...
- All changes to typescript-go submodule should be as minimal as possible, and performed through the patches in the `_patches/typescript-go/` folder. The `typescript-go/` directory is reset during check runs, so direct edits there are ephemeral unless captured in `_patches/typescript-go/`.
