...
| Language Service name | `catchDieToOrDie` |
| Oxlint name | `effecttsgo/catch-die-to-or-die` |
...
/**
  ^^^^^^^^^^^^ effecttsgo(catch-die-to-or-die): `Effect.orDie` expresses escalating every typed failure into a defect more directly than `Effect.catch` with an identity-forwarding `Effect.die` handler.
*/
)
```
