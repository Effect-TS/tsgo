...
test("generates sorted native tsgolint Effect rules", () => {
  ...
  assert.match(output, /etsoxlintrunner\.RunRuleAndReport/)
  assert.match(output, /Message: utils\.GetDiagnosticMessage/)
  assert.match(output, /Report: func\(diagnostic etsoxlintrunner\.ReportedDiagnostic\)/)
  assert.match(output, /related\.FileName == ctx\.SourceFile\.FileName\(\)/)
  assert.match(output, /Range: related\.Range/)
  assert.match(output, /LabeledRanges: labeledRanges/)
  assert.match(output, /ctx\.ReportDiagnosticWithSuggestions/)
  assert.match(output, /reported := diagnostic\.Suggestions\(\)/)
  assert.match(output, /FixesArr: fixes/)
