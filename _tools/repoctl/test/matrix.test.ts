...
import { buildTypeScriptTestMatrix } from "../src/matrix.ts"
...
test("builds a deterministic matrix for every unique TypeScript component", () => {
  ...
  assert.deepEqual(buildTypeScriptTestMatrix(upstream), {
    include: [
      { name: "6.9.0", version: "6.9.0", branch: "", repoctl: false },
      { name: "latest", version: "7.0.2", branch: "generated/latest", repoctl: false },
      { name: "next", version: "7.1.0-dev", branch: "", repoctl: true }
    ]
  })
})
...
test("combines channel labels when latest and next resolve to the same component", () => {
  ...
  assert.deepEqual(buildTypeScriptTestMatrix(upstream), {
    include: [{ name: "latest+next", version: "7.0.2", branch: "generated/latest", repoctl: true }]
  })
})
