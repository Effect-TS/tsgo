...
func main() {
	...
	// Generate etscore/version_generated.go from _packages/tsgo/package.json
	generateEffectVersionConst()
}

func generateEffectVersionConst() {
	pkgJSON, err := os.ReadFile("_packages/tsgo/package.json")
	if err != nil {
		log.Fatalf("error reading _packages/tsgo/package.json: %v", err)
	}
	...
	if err := json.Unmarshal(pkgJSON, &pkg); err != nil {
		log.Fatalf("error parsing _packages/tsgo/package.json: %v", err)
	}
	if pkg.Version == "" {
		log.Fatal("version field is empty in _packages/tsgo/package.json")
	}

	content := fmt.Sprintf(`// Code generated by _tools/gen_shims. DO NOT EDIT.

package etscore

const EffectVersion = %q
`, pkg.Version)

	if err := os.WriteFile("etscore/version_generated.go", []byte(content), 0644); err != nil {
		log.Fatalf("error writing etscore/version_generated.go: %v", err)
	}
}
